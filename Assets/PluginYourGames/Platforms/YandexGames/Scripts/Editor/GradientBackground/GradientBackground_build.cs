...
namespace YG.EditorScr.BuildModify
{
 ...
 $"{InfoYG.CORE_FOLDER_YG2}/Platforms/YandexGames/Scripts/Editor/GradientBackground/GradientBackground.css";
            ...
            if (infoYG.Templates.backgroundImgFormat == InfoYG.TemplatesSettings.BackgroundImageFormat.Gradient)
            {
                ...
                textCopy =
 textCopy.Replace("color1", ConvertToRGBA(infoYG.Templates.gradientBackgroundByLoadGame.color1));
                textCopy =
 textCopy.Replace("color2", ConvertToRGBA(infoYG.Templates.gradientBackgroundByLoadGame.color2));

                if (infoYG.Templates.gradientBackgroundByLoadGame.radial == false)
                {
                    textCopy = textCopy.Replace("radial-gradient", "linear-gradient");
                    textCopy =
 textCopy.Replace("circle", $"{infoYG.Templates.gradientBackgroundByLoadGame.angleInclination}deg");
                }
            ...
            if (infoYG.Templates.fixedAspectRatio && infoYG.Templates.fillBackground)
            {
                ...
                textCopy =
 textCopy.Replace("color1", ConvertToRGBA(infoYG.Templates.gradientBackgroundByAspectRatio.color1));
                textCopy =
 textCopy.Replace("color2", ConvertToRGBA(infoYG.Templates.gradientBackgroundByAspectRatio.color2));

                if (infoYG.Templates.gradientBackgroundByAspectRatio.radial == false)
                {
                    textCopy = textCopy.Replace("radial-gradient", "linear-gradient");
                    textCopy =
 textCopy.Replace("circle", $"{infoYG.Templates.gradientBackgroundByAspectRatio.angleInclination}deg");
                }
